...
function listQuotationValues(){ 
  $.ajax({
    url: "../admin/controllers/c_list-quotation-values.php",
    method: "POST",
    datatype: "JSON",
        ...
        <tr id="item-${e.id}">
          <td>
            <div class="cont-dashCamel__cControlsList--cC--cTable--cControl">
              ...
              <input type="number" id="" name="" class="cont-dashCamel__cControlsList--cC--cTable--cControl--input" value="${e.data_value}">
            </div>
          </td>
        </tr>
        `;
...
$(document).on('click', '#btnupdate-quotationvalues', function(e){
  ...
  $.ajax({
    url: "../admin/controllers/c_update-quotation-values.php",
    method: "POST",
    data: formdata,
    contentType: false,
    cache: false,
