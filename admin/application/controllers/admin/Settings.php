...
//            $mailbox = config_item('config_mailbox');
    ...
    public function utilidad_transporte_saved(){
        if( $data['value'] = $this->input->post('utilidad_transporte') ){
            $key = 'utilidad_transporte';
            if( $utilidad = $this->db->where('config_key', $key)->update('tbl_config', $data) ):
                $type = "success";
                $message = 'Utilidad de Transporte Modificada con Exito';
            else:
                $type = "error";
                $message = 'Error al intentar modificar la utilidad de Transporte.';
            endif;
            
        }else{
            $type = "error";
            $message = 'Error!! </br> Campo Vacio.'; 
        }
        set_message($type, $message);
