...
function inicializar() {
    ...
    $('#frm_cotizacion').submit(function(e){
      ...
      .done(function(data) {
        ...
        $('#contenedor_marco').css({'height':'470px'});
              ...
              <div id="dt_container">
                ...
                <div id="marc_opc_serv">
                  <div id="opc_si_servicio">
                    OPCION 1<br>AGREGAR SERVICIOS
                  </div>
                  <div id="opc_no_servicio">
                    OPCION 2<br>NO AGREGAR SERVICIOS
                  </div>
                  <input type="hidden" id="text_tip_serv" name="text_tip_serv">
                </div>
                <div id="si_servicios">
                  <h2>Servicios</h2>
                ...
                <div align="center">
                  <div id="dowload_cotizacion" class="button" rel_cotizacion="${data[0].user_cotizacion_id}">Calcular Flete
                  </div>
                </div>
                `);
...
        }
  ...
  if ($('input:checkbox[name=gast_portuario_aduana]').is(':checked') ) {
      ...
      ca=140;
      go=45;
      gas_por_alm_aduana=(ewb+vb+al_ref+gm+thc+dc+ca+go);
      console.log("gas_por_alm_aduana =>"+gas_por_alm_aduana);
  ...
  if ($('input:checkbox[name=gast_seg_merc]').is(':checked') ) {
    ...
    var valor_impuesto;
     ...
     if (txt_pre_carga>15000){
        valor_impuesto=txt_pre_carga*0.05;
     }else{
        valor_impuesto=60;
     }
     console.log("valor_impuesto =>"+valor_impuesto);
  }else{
    ch4=0;
  }
  ...
  .done(function(data) {
     ...
     if(data!=null){
        ...
        $('#fase3').html('<h2>Paso 4. Presupuesto</h2><div id="marc_det_resumen_presupuesto"><div id="tit_det_rPresupuesto1">ID: 00000000000'+data[0].user_cotizacion_id+'</div><div id="tit_det_rPresupuesto2">'+data[0].puerto_origen+' →'+data[0].puerto_destino+'</div><div id="tit_det_rPresupuesto3"><div id="marc_rPresupuesto3_izq"><div class="item_detalles">Transporte</div><div class="item1_detalles">'+data[0].tipo_flete+'</div><div class="item_detalles">Tipo</div><div class="item1_detalles">CONTENEDOR COMPLETO</div><div class="item_detalles">Cantidad</div><div class="item1_detalles" id="cont_can"></div><div class="item_detalles">Gasto portuario y Almacenamiento Aduanero</div><div class="item1_detalles"><div id="per_adu_sn"></div></div><div class="item_detalles">Tipo de producto</div><div class="item1_detalles">'+data[0].tipo_producto_id+'</div><div class="item_detalles">Impuestos:</div><div class="item1_detalles">'+data[0].importacion_estado+'</div></div><div id="marc_rPresupuesto3_der"><div class="item_detalles">Valor de mercancía</div><div class="item1_detalles">'+data[0].valor_producto+'</div><div class="item_detalles">Impuestos de Aduana</div><div class="item1_detalles"><div id="imp_adu_sn"></div></div><div class="item_detalles">Transporte a Domicilio</div><div class="item1_detalles">'+data[0].trans_origen+' -'+data[0].trans_destino+'</div><div class="item_detalles">Seguro de Mercancia:</div><div class="item1_detalles"><div id="seg_merca"></div></div><div class="item_detalles">Permiso Gubernamental Adicional:</div><div class="item1_detalles">MINAGRI / SENASA</div></div></div><div id="tit_det_rPresupuesto4">Validez de tarifa: 7 may - 16 may</div><div id="tit_det_rPresupuesto3"><div id="servicio_completo"></div><div id="servicio_incompleto"></div><div id="tit_det_rPresupuesto3"><div align="center"><a id="btn_descargar_coti" target="_blank" download href="views/modules/pdf.php?ID='+data[0].user_cotizacion_id+'" class="button">Descargar Cotizacion</a></div></div>')
      ...
      if (data[0].servicios_4==4){
        ...
        var subt_tot=0;
        var total_imp=0;
        subt_tot=data[0].sub_total_containers;
        total_imp=(parseFloat(gas_por_alm_aduana)+parseFloat(impuesto_imp)+parseFloat(valor_impuesto)+parseFloat(subt_tot));
        console.log("total Impuests =>"+total_imp);

        
        if (data[0].servicio_estado=="SI"){
          $("#servicio_incompleto").html('');
          $("#servicio_completo").html('<div id="marc_rPresupuesto3_izq"><div class="item_detalles_sub">Incluye</div><div class="item_detalles_sub">FLETE MARITIMO</div><div class="item_detalles_sub">THCD (Manejo de naviera en Destino)</div><div class="item_detalles_sub">VISTOS BUENOS</div><div class="item_detalles_sub">GATE IN(Recepción de contenedor vacio en el puerto</div><div class="item_detalles_sub">ALMACEN ADUANERO</div><div class="item_detalles_sub">HONORARIOS DE AGENCIA DE ADUANA</div><div class="item_detalles_sub">TRANSPORTE A FABRICA IMPORTADOR</div><div class="item_detalles_sub">SEGURO DE MERCANCIA</div></div><div class="costo_tot">'+total_imp+',<sup>00</sup> USD</div>');

        }  
        if (data[0].servicio_estado=="NO"){
          $("#servicio_completo").html('');
          $("#servicio_incompleto").html('<div id="marc_rPresupuesto3_izq"><div class="item_detalles_sub">Incluye</div><div class="item_detalles_sub">FLETE MARITIMO</div><div class="item_detalles_sub">THCD (Manejo de naviera en Destino)</div><div class="item_detalles_sub">No incluye</div><div class="item_detalles_sub">VISTOS BUENOS</div><div class="item_detalles_sub">GATE IN(Recepción de contenedor vacio en el puerto</div><div class="item_detalles_sub">ALMACEN ADUANERO</div><div class="item_detalles_sub">HONORARIOS DE AGENCIA DE ADUANA</div><div class="item_detalles_sub">TRANSPORTE A FABRICA IMPORTADOR</div><div class="item_detalles_sub">SEGURO DE MERCANCIA</div></div><div class="costo_tot">'+total_imp+',<sup>00</sup> USD</div></div>');
      ...
      console.log("complete");
      $('.cajacalc').css({'width':'980px'});
      $('.cajacalc').css({'margin':'0px 0px 0px -164px'});
    }); 
...
function show_envio_domicilio(){
  ...
  if ($('input:checkbox[name=gast_trans_dom]').is(':checked') ) {
    ...
    $('#contenedor_marco').css({'height':'1660px'});
  }else{
    console.log("no");
...
function show_con_servicio(){
  ...
  $('#marc_datos_contacto').css({'display':'block'});
  $('#contenedor_marco').css({'height':'1110px'});
  $('#dowload_cotizacion').css({'display':'block'});
  $('#text_tip_serv').val('SI');
}
function show_sin_servicio(){
  console.log('show_sin_servicio');
  $('#si_servicios').css({'display':'none'});
  $('#contenedor_marco').css({'height':'910px'});
  $('#marc_datos_contacto').css({'display':'block'});
  $('#dowload_cotizacion').css({'display':'block'});
